...
export function AnalyticsClient({ id }: { id: string }) {
  ...
  const kpis: { label: string; value: string; icon: React.ComponentType<{ className?: string }>; tone: string }[] = [
    { label: "Convidados confirmados", value: `${confirmedPct}%`, icon: Users, tone: "text-gold-500" },
    { label: "Custo previsto", value: formatBRL(data.finance.plannedCents), icon: Wallet, tone: "text-money-600" },
    { label: "Fornecedores ativos", value: String(data.vendors.length), icon: Store, tone: "text-slate-700" },
    {
